...
- (void)handleOpenURL:(NSURL *)URL;
{
    ...
    IwTrace(FACEBOOK_VERBOSE, ("handleOpenURL:%s", _url));

    if (strstr(_url, "fb") == _url)
    {
        // Call FB controller method
        BOOL handled = [m_session handleOpenURL:URL];
    }
    else
        IwTrace(FACEBOOK_VERBOSE, ("Didn't match URL"));
}
...
int32 _handleOpenURL(void* instance, void* systemData, void* userData)
{
    NSURL *url = (NSURL *)userData;
    s3eFBSessionDelegate *fbSession = (s3eFBSessionDelegate *)instance;
