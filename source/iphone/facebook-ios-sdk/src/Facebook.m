...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
  ...
  // IWLABS: Don't try auth methods that rely on multitasking
  // since this is currently broken in the .lib version of the 
  // the iphone loader - t o b e
  tryFBAppAuth = NO;
  trySafariAuth = NO;
  ...
  if ([device respondsToSelector:@selector(isMultitaskingSupported)] && [device isMultitaskingSupported]) {
    if (tryFBAppAuth) 
    {
      ...
      // IWLABS: Workaround issue where openURL doesn't
      // work as expected in .lib build of loader most like
      // due to a non-linked library that actually performs
      // this actions - t o b e
      BOOL willRespond = [theApp respondsToSelector:@selector(canOpenURL:)];
      BOOL canOpen = [theApp canOpenURL:[NSURL URLWithString:fbAppUrl]];

      if (willRespond && canOpen)
      {
        NSLog(@"Trying FB app");
    ...
    // IWLABS: Multitasking is currently broken for I3D.
    // Re-enable (remove the NO) when we fix it. This auth method requires
    // the app to stick around whilst the user authorizes in Safari
    // -- t o b e
    if (trySafariAuth && !didOpenOtherApp && NO) 
    {
      NSLog(@"Trying Safari");
