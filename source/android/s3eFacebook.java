...
class s3eFacebook
{
    ...
    class s3eFBSession
    {
        ...
        {
            AirplayActivity.m_Activity.runOnUiThread(new Runnable() {
                public void run() {
                    m_Facebook.authorize(AirplayActivity.m_Activity, permissions, Facebook.FORCE_DIALOG_AUTH, new LoginDialogListener());
                }
            });
        ...
        public int Logout()
        {
            try
            {
                m_Facebook.logout(AirplayActivity.m_Activity);
            }
            catch (Exception e)
            {
    ...
    class s3eFBDialog
    {
        ...
        int Show()
        {
            AirplayActivity.m_Activity.runOnUiThread(new Runnable() {
                public void run() {
                    m_Session.m_Facebook.dialog(AirplayActivity.m_Activity, m_Action, m_Params, new FBDialogListener());
                }});
            return S3E_RESULT_SUCCESS;
        }
